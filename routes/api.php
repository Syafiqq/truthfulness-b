...
$router->group(['namespace' => 'Student', 'prefix' => '/student'], function () use ($router) {
    ...
    $router->group(['middleware' => 'c.jwt.auth.student'], function () use ($router) {
        ...
        $router->group(['middleware' => 'valid.student.profile'], function () use ($router) {
            ...
            $router->get('/course/start/{question:\d+}', ['middleware' => ['course.open.available', 'valid.question'], 'uses' => 'Course@startEdit', 'as' => 'student.course.start.get']);
            $router->get('/course/navigation/{question:\d+}', ['middleware' => ['course.open.available', 'valid.question'], 'uses' => 'Course@navigation', 'as' => 'student.course.navigation.get']);
            $router->patch('/course/start/{question:\d+}', ['middleware' => ['course.open.available', 'valid.question', 'valid.answer'], 'uses' => 'Course@startUpdate', 'as' => 'student.course.start.patch']);
